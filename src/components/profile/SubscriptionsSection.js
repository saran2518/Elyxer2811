...
function categorize(feature) {
    ...
    if (/(discover|search|browsing|vibes|invites)/.test(f))
        return "discover";
    if (/(virtual date|moments|post|vibed you|invited you|interact)/.test(f))
        return "connect";
    if (/(visibility|unlock|control|private)/.test(f))
        return "visibility";
    if (/(profile generation|studio)/.test(f))
        return "studio";
    return "discover";
}
const GROUP_META = {
...
const SubscriptionsSection = () => {
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
      {/* Buy Extras */}
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-center gap-1.5">
          {plans.map((_, i) => (<button key={i} onClick={() => scrollToPlan(i)} aria-label={`Go to ${plans[i].title}`} className={`h-1.5 rounded-full transition-all duration-300 ${activePlanIndex === i
                ? "w-4"
                : "w-1.5 bg-muted-foreground/30 hover:bg-muted-foreground/50"}`} style={activePlanIndex === i ? { background: "var(--gradient-warm)" } : undefined}/>))}
        </div>

        <div className="-mx-4">
          <div ref={plansScrollRef} className="flex gap-3 overflow-x-auto px-4 pb-2 snap-x snap-mandatory scrollbar-hide" style={{ WebkitOverflowScrolling: "touch" }}>
            {plans.map((plan, i) => (<PlanCard key={i} index={i} plan={plan} isActive={activeSub?.plan === plan.planKey} activeSub={activeSub}/>))}
          </div>
        </div>
      </div>
      ...
      <p className="px-1 text-center text-[10px] leading-relaxed text-muted-foreground">
        ...
        <button onClick={() => navigate("/terms")} className="inline text-primary underline underline-offset-2 hover:text-primary/80">
          Terms & Conditions
        </button>
      </p>
    </motion.div>);
};
function PlanCard({ plan, isActive, activeSub, index, }) {
    const navigate = useNavigate();
    ...
    return (<div data-plan-card data-index={index} className={`relative rounded-[20px] border ${borderClass} bg-card flex flex-col snap-center shrink-0 overflow-hidden`} style={{ width: "78vw", maxWidth: 300, ...shadowStyle }}>
      {/* Warm wash */}
      {!isActive && (<div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(to bottom, hsl(var(--primary) / 0.08) 0%, hsl(var(--primary) / 0.03) 60%, transparent 100%)" }}/>)}

      {/* Badge row */}
      <div className="relative flex justify-end gap-1.5 px-2.5 pt-1">
        {isActive && (<span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-primary text-primary-foreground">
            ACTIVE
          </span>)}
        {plan.badge && !isActive && (<span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${plan.badgeClass || ""}`} style={!plan.badgeClass ? { background: "var(--gradient-warm)", color: "hsl(var(--primary-foreground))" } : undefined}>
            {plan.badge}
          </span>)}
      </div>
      ...
      <div className={`relative px-3 ${plan.badge || isActive ? "pt-1" : "pt-2.5"} pb-3 flex flex-col flex-1`}>
        ...
        <div className="flex flex-col items-center text-center mb-2">
          ...
          {isActive && activeSub && (<p className="mt-1 text-[11px] text-muted-foreground">
              Renews <span className="font-medium text-foreground">{activeSub.nextBilling}</span>
            </p>)}
          {!isActive && (<div className="mt-0.5 flex items-baseline gap-1">
        ...
        <div className="mb-2 space-y-2">
          {!isActive && (<Button onClick={() => navigate(`/subscribe?plan=${plan.planKey}`)} className={`w-full rounded-2xl gap-1.5 h-8 text-[12px] font-medium ${plan.ctaClass || ""}`} style={plan.ctaStyle}>
        ...
        <button onClick={() => navigate(`/subscribe?plan=${plan.planKey}`)} className="mt-2 flex items-center justify-center gap-1 text-[10px] font-medium text-primary hover:text-primary/80 transition-colors">
          View all benefits
...
function PlanCategoryList({ features }) {
    const groups = useMemo(() => {
        const buckets = { discover: 0, connect: 0, visibility: 0, studio: 0 };
    ...
    return (<div className="space-y-1">
      {groups.map(([key, count]) => {
            ...
            return (<div key={key} className="flex items-center gap-2 rounded-lg border border-border/20 px-2 py-1.5" style={{ background: "hsl(45 40% 96% / 0.4)" }}>
            <div className="h-5 w-5 rounded-full flex items-center justify-center shrink-0" style={{ background: "hsl(41 70% 64% / 0.22)", color: "hsl(32 70% 36%)" }}>
              {meta.icon}
            </div>
...
function PurchaseItem({ icon, label, count, onClick }) {
    return (<div className="group relative rounded-[20px] border border-border/40 bg-card p-3 pb-4 flex flex-col items-center transition-all duration-300 hover:-translate-y-0.5" style={{ boxShadow: "0 4px 20px -4px hsl(var(--primary) / 0.08)" }}>
      <div className="h-8 w-8 rounded-xl flex items-center justify-center transition-transform duration-300 group-hover:scale-110" style={{ background: "var(--gradient-warm)", boxShadow: "0 2px 8px -2px hsl(var(--primary) / 0.3)" }}>
      ...
      <button onClick={onClick} aria-label={`Buy more ${label}`} className="absolute -bottom-2 left-1/2 -translate-x-1/2 h-6 w-6 rounded-full bg-card border border-border/40 flex items-center justify-center text-primary shadow-sm transition-all hover:scale-110 active:scale-95" style={{ boxShadow: "0 2px 8px -2px hsl(var(--primary) / 0.2)" }}>
        <Plus className="h-3 w-3" strokeWidth={3}/>
      </button>
    </div>);
}
function ExtraTierCard({ tier, unit, onSelect }) {
    const highlighted = !!tier.badge;
    return (<div className={`rounded-2xl border-2 ${highlighted ? "border-primary/40" : "border-border/30"} bg-card flex flex-col snap-center shrink-0 p-4`} style={{ width: "60vw", maxWidth: 220, boxShadow: highlighted ? "var(--shadow-warm)" : "var(--shadow-card)" }}>
      <div className="flex justify-end h-5">
        {tier.badge && (<span className="text-[9px] font-bold px-2.5 py-0.5 rounded-full" style={{ background: "var(--gradient-warm)", color: "hsl(var(--primary-foreground))" }}>
            {tier.badge}
          </span>)}
      </div>
...
function PlanFeature({ included, label }) {
    return (<div className="flex items-center gap-1.5 text-[12px]">
      {included ? (<div className="h-4 w-4 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
          <Check className="h-2.5 w-2.5 text-primary"/>
        </div>) : (<div className="h-4 w-4 rounded-full bg-muted/60 flex items-center justify-center shrink-0">
