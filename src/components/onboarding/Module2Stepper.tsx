...
import { Cake, User, Users, CheckCircle } from "lucide-react";

const STEPS = [
  { label: "Birthday", icon: Cake },
  { label: "Gender", icon: User },
  { label: "Pronouns", icon: Users },
  { label: "Done", icon: CheckCircle },
];

interface Module2StepperProps {
  currentStep: number;
  progressPercent: string;
  doneActive?: boolean;
}

const Module2Stepper = ({ currentStep, progressPercent, doneActive }: Module2StepperProps) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
              ...
              style={
                isActive || isCompleted
                  ? {
                      ...
                      boxShadow: "0 4px 14px -3px hsl(32 70% 36% / 0.35)",
                    }
                  : undefined
              }
            >
